...
document.querySelectorAll(".product").forEach((item, index) => {
  ...
  for (let i = 0; i < children.length; i++) {
    if (children[i].getAttribute("class") === "product-title") {
      !selectedItems.includes(children[i].textContent)
...
document.querySelectorAll(".product-button").forEach((item, index) => {
  item.addEventListener("click", (element) => {
    ...
    for (let i = 0; i < children.length; i++) {
      children[i].getAttribute("class") === "product-title"
        ? (title = children[i].textContent)
        : children[i].getAttribute("class") === "product-price"
        ? (price = children[i].textContent)
        : "";
    }
    ...
    adobeDataLayer.push({
      ...
      cart: {
        productCategory:
          title === "HP Pavilion" ? "Hp Laptops" : "dell Laptops",
