...
window.addEventListener('load',()=>{
    adobeDataLayer.push({
        ...
        "timeStamp":{
            ...
            "time": currDate.getHours() + ":" + currDate.getMinutes() + ":0" + currDate.getSeconds(),
 ...
 document.querySelectorAll("button").forEach((item,index)=>{item.addEventListener("click",(e)=>{
     if(e.target.id!="submit" && e.target.id!="addToCart"){
                adobeDataLayer.push({
                    link:{
                        "contentSection":e.target.id==="submit"?"form":e.target.id==="readMore"?"description":e.target.id==="addToCart"?"productComponent":"",
                        "contentSubsection":"",
                        "linkText":e.target.textContent,
                        "linkComponent":e.target.id==="submit"?"form":e.target.id==="readMore"?"description":e.target.id==="addToCart"?"productComponent":"",
                        "linkIndex":index+1,
                    },
                    "metric":e.target.id==="addToCart"? "aa-action,aa-addtocart":"aa-action",
                    "event":"aa-action"
                });
            }
...
document.querySelectorAll("a").forEach((item,index)=>{
    ...
    item.addEventListener('click',(e)=>{
        ...
        adobeDataLayer.push({
            ...
            "metric":e.target.parentNode.getAttribute("id")==="footer"?"aa-link,aa-footer,aa-social":"aa-link,aa-banner-clickout",
            "event":"aa-link"
        });
    });
});
